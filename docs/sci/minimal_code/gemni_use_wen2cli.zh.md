...
2.  权益范围：AI Pro 订阅的权益仅限于网页版 `gemini.google.com` 及集成的 Google 应用，不适用于 API 或 CLI。
...
您的 AI Pro 订阅状态仅影响网页版的限额，不影响通过账户登录的 CLI。
...
使用免费 API 密钥无法调用 Pro 模型。您必须使用与启用了结算功能的 Google Cloud 项目关联的 API 密钥。
