...
include 相当于在预处理把 b.h 的内容直接插入到 main.cpp 和 b.cpp 里。这样 main.cpp 和 b.cpp 都能看到对 msg 的声明，编译器就知道 msg 是什么了。 所以头文件其实给的一个可以互相嵌套使用的声明域，不需要手动写所有的声明。可以减少重复，减少依赖（或者说是隐藏依赖）。
