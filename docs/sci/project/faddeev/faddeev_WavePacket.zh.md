...
### 致命缺陷：三体 Lippmann–Schwinger 方程的病态本质
...
- 第一次作用 $V_{23}$：势 $V_{23}$ 只依赖于 $\vec{\rho}_1$，它只作用于子系统 (23)。它可以使氘核保持束缚，也可以使其激发或破裂，但它完全不影响粒子 1 的运动状态。
...
数学后果：Fredholm 积分方程理论的核心定理之一，就是只有当积分核是紧致时，方程解的唯一性才能得到保证。对于我们这里的非紧致核，Fredholm 理论不适用，方程的解不唯一。
