...
- 零特征值的特征向量满足 $[H, C] = 0$，即与 $H$ 对易的守恒量。
- 非零特征值的特征向量就是升降算符，特征值的大小给出步长。
- 在李代数语言中，$\mathrm{ad}_H$ 的特征分解就是根空间分解（root space decomposition）：Cartan 元 $H$ 的伴随作用把李代数分解为根空间，根向量就是升降算符，根就是步长。
...
物理上，角动量分量不能超过角动量的总大小。代数上，这是 $\mathfrak{su}(2)$ 作为紧致李群的李代数，其有限维不可约表示必然是有限维的——Casimir 值固定后，权（即 $m$ 值）的范围是有限的。
...
> 核心要点：升降算符的谱是否有界，不取决于升降算符本身，而取决于代数结构中正定性约束的数量。一个正定性条件（$a^\dagger a \geq 0$）给出单侧截断；若 Casimir 提供第二个正定性条件（$\mathbf{L}^2 - L_z^2 \geq 0$），则双侧截断，表示变为有限维。
