# 有效力程理论与 Levinson 系统化

前一篇 `jost_analyticity.zh.md` 把分波振幅 $f_l(k)$ 写成 Jost 函数 $F_l^\pm(k)$ 的比，把束缚态、虚态、共振、阈值零能态四类奇性都归到 $F_l^+(k)$ 在复 $k$ 平面的零点上。Levinson 定理在那里以论域原理的轮廓出现：$\delta_l(0) - \delta_l(\infty)$ 的整体值由束缚态数控制。
...
记号沿用 `jost_analyticity.zh.md:79` 的远场展开 (F-asy) 与 `jost_analyticity.zh.md:87` 的 Wronskian 定义 (F-W)。短程实势下 $F_l^+(k)$ 在上半 $k$ 平面解析（含实轴），$k = 0$ 是实轴上的一个普通点（除非碰巧 $F_l^+(0) = 0$，此为零阈值情形，见后）。所以 $F_l^+(k)$ 可以在 $k = 0$ 附近做 Taylor 展开

$$
F_l^+(k) = F_l^+(0) + k\, F_l^{+\prime}(0) + \frac{k^2}{2}\, F_l^{+\prime\prime}(0) + O(k^3) \tag{F-Taylor}
$$

对实势，由 (F-conj) `jost_analyticity.zh.md:117` 即 $F_l^+(k)^* = F_l^-(k^*)$，在实 $k$ 上 $F_l^-(k) = F_l^+(k)^*$。把 (F-asy) 与 (F-conj) 合并得 (F-phase) `jost_analyticity.zh.md:131`：实 $k > 0$ 上 $F_l^+(k) = |F_l^+(k)|\, e^{-i\delta_l(k)}$。

为了把 $\delta_l(k)$ 的低能行为从 (F-Taylor) 直接读出，需要弄清 $F_l^{+\prime}(0)$、$F_l^{+\prime\prime}(0)$ 等系数的实虚结构。再次用 (F-conj)：在 $k$ 实数邻域附近 $F_l^+(k)^* = F_l^-(k) = F_l^+(-k)$（最后一步用 $F_l^-(k, r) = F_l^+(-k, r)$，`jost_analyticity.zh.md:70` 的对称性）。这给出
...
self-derive。从 (F-W) 出发把 $F_l^+$ 的归一化展开。零势极限下 (F-W) 给 $F_l^+ \equiv 1$（见 `jost_analyticity.zh.md:101`）。短程势下，$F_l^+(k)$ 的小 $k$ 行为可写

$$
F_l^+(k) = 1 - i k^{2l+1}\, a_l + b_l\, k^{2l+2} + O(k^{2l+3}) \tag{F-low}
$$

证明 (F-low) 的关键是 (F-W) 中 $\phi_l(k, r)$ 与 $f_l^+(k, r)$ 的低能展开。规则解 (phi-0) `jost_analyticity.zh.md:50` 在原点行为 $r^{l+1}/(2l+1)!!$ 与 $k$ 无关，因此 $\phi_l(k, r)$ 是 $k^2$ 的整函数（Newton 12.1 节标准结论）：$\phi_l(k, r) = \phi_l^{(0)}(r) + k^2 \phi_l^{(2)}(r) + \ldots$，所有偶幂。Jost 解 $f_l^+(k, r) \to e^{ikr}$ 远场行为，小 $k$ 下 $f_l^+(k, r) = u_l^+(r) + i k\, w_l^+(r) + O(k^2)$，其中 $u_l^+(r) = \lim_{k \to 0} f_l^+(k, r)$ 满足零能径向方程，远场 $u_l^+(r) \to 1$（自由 $l$ 阶 Riccati-Hankel 在 $k = 0$ 退化），$w_l^+(r) \to r$ 远场。
...
self-derive（论域原理路径）。从 Jost 函数的论域恒等式 (argP) `jost_analyticity.zh.md:208`：上半 $k$ 平面零点数 $n_0 = (1/2\pi i)\oint d\log F_0^+$。把围道取实轴 $-R$ 到 $R$ 加上半圆。实轴段贡献 $-(\delta_0(R) - \delta_0(-R))/\pi = -2(\delta_0(R) - \delta_0(0))/\pi$（用 $\delta_0(-R) = -\delta_0(R)$ 实势对称）。半圆段 $R \to \infty$ 给零（短程势）。$R \to \infty$ 取极限：
...
`jost_analyticity.zh.md:208` 给了论域原理证明的轮廓。本节把它写完整，包含半整数修正。
...
第一步：上半平面零点结构。`jost_analyticity.zh.md:169` 证明了短程实势下 $F_l^+$ 上半平面零点必在正虚轴；正虚轴零点对应束缚态（`jost_analyticity.zh.md:163`）。所以 $R \to \infty$ 取极限 $N_l(\infty) = n_l$，第 $l$ 分波束缚态数。
...
第三步：半圆段。$|k| = R \to \infty$ 时 $F_l^+(k) \to 1$（短程势 Volterra 零阶项主导，`jost_analyticity.zh.md:227`），故 $\log F_l^+ \to 0$，半圆段贡献 $\int_{\Gamma_R} d\log F_l^+ \to 0$，相当于 $\delta_l(R) \to \delta_l(\infty)$。
...
证明完。这条定理把"低能相移 $\delta_l(0)$"与"束缚态数 $n_l$"连成一个整数关系，是相移代码最常用的 sanity check（见 `jost_analyticity.zh.md:241`）。
...
NN ${}^3 S_1$-${}^3 D_1$。两个分波 $S = l = 0$ 与 $D = l = 2$。Stapp 参数化下 $S$ 矩阵相移分别为 $\delta_S, \delta_D$ 与混合参数 $\epsilon_1$（mixing angle）。$a_S \approx 5.42$ fm（氘核束缚态），$a_D \approx 6.5$ fm³（量纲不同，$l = 2$）；混合参数低能 $\epsilon_1(k) \to k^2/(\ldots)$。详细多通道 ERE 见 `partial_wave_projection.zh.md:393` 耦合通道节，这里只给出形式。
...
Jost 函数远场展开 (F-asy) 与 Wronskian 定义 (F-W)：`jost_analyticity.zh.md:79` 与 `jost_analyticity.zh.md:87`。
$F_l^+$ 与相移的 (F-phase) 关系：`jost_analyticity.zh.md:131`。
论域原理证明 Levinson 定理的轮廓：`jost_analyticity.zh.md:208` 至 `jost_analyticity.zh.md:236`，本篇第 7 节把它写完整。
零阈值修正 (Levinson-mod)：`jost_analyticity.zh.md:250`。
分波 $T$ 矩阵与 $\delta_l$ 的 on-shell 关系：`partial_wave_projection.zh.md:340` 与 `partial_wave_projection.zh.md:369`。
分波 $S$ 矩阵 $S_l = e^{2i\delta_l}$：`partial_wave_projection.zh.md:378`。
...
${}^1 S_0$ 虚态机制：`jost_analyticity.zh.md:165`。
Coulomb 势上 Levinson 定理需修改的提示：`jost_analyticity.zh.md:243`。
...
- Coulomb-modified ERE。Coulomb 势上 Wigner 阈值变为 $\delta_l \sim -k^{2l+1}\, e^{-2\pi\eta}$（指数压制），ERE 改写为 Coulomb-distorted phase shift 的展开 $C_0^2(\eta) k\cot\delta_0 + 2\eta k\, h(\eta) = -1/a_C + r_C k^2/2 + \ldots$（Bethe-Salpeter；Bethe 1949）。$h(\eta) = \mathrm{Re}\,\psi(i\eta) - \ln\eta$。具体推导与 `coulomb_scattering.zh.md` 的畸变波接口。
